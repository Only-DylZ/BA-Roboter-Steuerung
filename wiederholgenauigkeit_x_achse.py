...
"""
================================================================================
 Wiederholgenauigkeitstest  —  igus ReBeL 6-DOF  (X-Achse)
================================================================================

Prueft die (unidirektionale) Wiederholgenauigkeit des Roboters entlang der
Basis-X-Achse: Eine feste Pruefkoordinate P wird immer aus derselben Richtung
...
Einrichten (VOR jedem Lauf):
  1. Roboter in iRC von Hand an die zu pruefende Position fahren. Dabei
     ...
     muss parallel zur Y-Z-Ebene stehen und in Richtung der positiven
     X-Achse schauen. Die dabei in iRC angezeigten A/B/C-Werte mit
...
Bewegungsablauf (10 Zyklen):
  - Start: Startposition anfahren (= Pruefkoordinate, aber 200 mm/20 cm in
    -X versetzt).
  - Je Zyklus:
      1. 200 mm in +X zur Pruefkoordinate P fahren (Anfahren der Flaeche).
      2. 2 s warten (Position von Hand vermessen).
      3. 200 mm in -X zurueck zur Startposition fahren.
      4. 10 s warten.
...
# entlang der Pruefachse (X). Die Startposition liegt bei X = CHECK_X -
# APPROACH_DISTANCE; von dort wird in +X-Richtung auf P zugefahren.
APPROACH_DISTANCE = 350.0     # 35 cm

# --- Werkzeug-Orientierung [Grad] ---  Vordere plane Flaeche des Greifers
# parallel zur Y-Z-Ebene, schauend in +X-Richtung.
...
# senkrecht nach unten). Rechnerisch ergibt A=0, B=90, C=0 eine Blickrichtung
# von +X mit derselben "oben/unten"-Referenz wie beim Nach-unten-Zeigen.
# WICHTIG: Vor dem ersten automatischen Lauf in iRC anfahren und mit der
# geforderten Ausrichtung (Flaeche parallel Y-Z, Blick +X) visuell
...
def run_wiederholgenauigkeit_x(robot, check_xyz, approach_distance, tool,
                                cart_vel=250.0, wait_check=2.0, wait_start=10.0,
                                num_cycles=10, log=_default_log, check=None):
    """Faehrt die Pruefkoordinate entlang +X num_cycles mal linear an.

    check_xyz         : (x, y, z) der Pruefkoordinate P [mm].
    approach_distance : Abstand [mm] der Startposition von P entlang X.
    ...
    a, b, c = tool
    cx, cy, cz = check_xyz
    start = (cx - approach_distance, cy, cz)
    ziel  = (cx, cy, cz)
    ...
    for n in range(1, num_cycles + 1):
        ...
        isleep(wait_check)
        log("  -> zurueck zur Startposition (-X)")
        check(); robot.move_cartesian(*start, a, b, c, velocity=cart_vel)
...
def main():
    ...
    print("=" * 70)
    print("  Wiederholgenauigkeitstest -- X-Achse  (igus ReBeL 6-DOF)")
    print(f"  Pruefkoordinate P : ({CHECK_X}, {CHECK_Y}, {CHECK_Z})")
    print(f"  Anfahrweg         : {APPROACH_DISTANCE:.0f} mm entlang X, "
          f"Anfahrtrichtung +X")
    ...
    input("\nWerte oben pruefen (insb. Pruefkoordinate und Werkzeug-"
          "Orientierung -- Flaeche muss parallel zur Y-Z-Ebene stehen und "
          "in +X-Richtung schauen). Arbeitsbereich frei? Enter zum Start, "
